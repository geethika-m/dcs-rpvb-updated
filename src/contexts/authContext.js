...
export function AuthProvider({ children }) {
    ...
    await database.loginRef
      ...
      .then((snapShot) => {
        snapShot.forEach((doc) => {
          ...
          const dateLock =
            doc.data().lockDate !== null ? doc.data().lockDate.seconds : 0;
    ...
    await database.usersRef
      ...
      .then((userRefSnapShot) => {
        userRefSnapShot.forEach((doc) => {
          ...
          if (doc.data().lastChangedPassword !== undefined) {
            const changeSecondsDifference =
              epochTimeNow - doc.data().lastChangedPassword.seconds;
            ...
            if (changeSecondsDifference > 7776000) {
              ...
              daysLeft = Math.ceil(
                90 - changeSecondsDifference / 60 / 60 / 24
              ).toString();
              alert(
                "User has " +
                  daysLeft +
                  " days to change your password before password expiration."
              );
            }
            localStorage.setItem(
              "dateOfLastChange",
              encryptData(doc.data().lastChangedPassword.seconds.toString())
            );
          } else {
            const createdSecondsDifference =
              epochTimeNow - doc.data().createdOn.seconds;
            ...
            if (createdSecondsDifference > 7776000) {
              ...
              daysLeft = Math.ceil(
                90 - createdSecondsDifference / 60 / 60 / 24
              ).toString();
              alert(
                "User has " +
                  daysLeft +
            ...
            localStorage.setItem(
              "dateOfLastChange",
              encryptData(doc.data().createdOn.seconds.toString())
            );
          }
      ...
      .catch((e) => {
        if (e.toString().includes("Archived")) {
          ...
          loginMsg =
            "ERROR: Password has Expired. Please reset your account password.";
        } else {
  ...
  const checkPasswordGeneration = async (password, email) => {
    try {
      ...
      const snapshot = await database.loginRef
        .where("email", "==", email)
        .get();
      ...
      for (const doc of snapshot.docs) {
        ...
        if (
          Array.isArray(hashedPasswordArray) &&
          hashedPasswordArray.every(bcrypt.getRounds)
        ) {
  ...
  const changePassword = async (oldPassword, newPassword) => {
    ...
    try {
      ...
      const checkMsg = await checkPasswordGeneration(
        newPassword,
        currentUser.email
      );
      ...
      if (checkMsg.includes("SUCCESS")) {
        ...
        const snapshot = await database.usersRef
